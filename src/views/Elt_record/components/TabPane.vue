<template>
  <div>
    ...
    >
      <!-- 参与人 -->
      <template v-if="type === 'Attend' && !!detailObj">
        <el-table-column
          prop="userName"
          align="center"
          label="学生"
        ...
        <el-table-column prop="" align="center" label="操作" width="120">
          <template slot-scope="{ row }">
            <el-button
              type="primary"
              size="mini"
              @click="handleAttendDetail(row.jobPlanId)"
            >
              查看该生
            </el-button>
          </template>
        ...
        >
          ...
          <span slot="footer" class="dialog-footer">
            <el-button type="primary" @click="selectCourse = false"
              >确 定</el-button
            >
          </span>
        </el-dialog>
      ...
      <template v-if="type === 'Paper'">
        ...
        <!-- <el-table-column min-width="150px" align="center" label="试卷编号">
          <template slot-scope="{ row }">
            <span>{{ row.testPaperSerialNo }}</span>
          </template>
        </el-table-column> -->
...
export default {
  ...
  created() {
    console.log("scheduleId--tab-pane", this.scheduleId, this.type + "12");
    this.getList();
  },
  watch: {
    detailObj() {
  ...
  methods: {
    async getList() {
      this.loading = true;
      if (this.type === "Attend") {
        this.list = this.detailObj.studentList;
        this.list = this.sliceList(this.list, 10)[this.listQuery.pageIndex - 1];
        console.log("参与人==>", this.list);
        this.total = this.detailObj.studentList.length;
        ...
        console.log("获取错题列表");
        let queryObj = {};
        queryObj.scheduleId = this.scheduleId;

        const wrongList = await getWrongQuestionList(queryObj);
        console.log("获取错题列表==>", wrongList);
        // if(wrongList.success){
